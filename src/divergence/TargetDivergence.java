...
public class TargetDivergence implements ActionDivergence {
	...
	private static final int[] OFFSETS = new int[] { 
		...
		4 };
	/** 
	 * The chance associated with each offset; the last value is left out
	 * since the chances must add up to 1.
	 */
	private static final double[] DEFAULT_CHANCES = new double[] { 
		0.5, 
		0.2, 0.1,
		0.08, 0.03,
		0.05, 0.02,
	 ...
	 * @param grid the grid over which the target moves.
	 */
	 ...
	 * @param grid the grid over which the target moves.
	 * @param chances the distribution over the offsets.
	 */
	...
	public Action divergeAction(Action action) {
		...
		// Otherwise, we add a random offset to the heading, and return this action.
